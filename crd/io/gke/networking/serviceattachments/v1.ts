...
export class KubernetesServiceAttachmentV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesServiceAttachmentV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "networking.gke.io/v1", kind: "ServiceAttachment", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesServiceAttachmentV1Config extends ManifestConfig {
  metadata: {
    annotations?: {
      [key: string]: string;
    };
