...
export class KubernetesClusterEphemeralReportV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesClusterEphemeralReportV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "reports.kyverno.io/v1", kind: "ClusterEphemeralReport", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesClusterEphemeralReportV1Config extends ManifestConfig {
  metadata: {
    annotations?: {
      [key: string]: string;
    };
