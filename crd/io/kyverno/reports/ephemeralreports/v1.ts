...
export class KubernetesEphemeralReportV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesEphemeralReportV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "reports.kyverno.io/v1", kind: "EphemeralReport", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesEphemeralReportV1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
