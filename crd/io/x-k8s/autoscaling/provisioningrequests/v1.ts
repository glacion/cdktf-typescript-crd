...
export class KubernetesProvisioningRequestV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesProvisioningRequestV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "autoscaling.x-k8s.io/v1", kind: "ProvisioningRequest", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesProvisioningRequestV1Config extends ManifestConfig {
  metadata: {
    annotations?: {
      [key: string]: string;
    };
