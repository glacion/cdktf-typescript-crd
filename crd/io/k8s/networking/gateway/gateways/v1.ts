...
export class KubernetesGatewayV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesGatewayV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "gateway.networking.k8s.io/v1", kind: "Gateway", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesGatewayV1Config extends ManifestConfig {
  metadata: {
    annotations?: {
      [key: string]: string;
    };
