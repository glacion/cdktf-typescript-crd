...
export class KubernetesClusterGeneratorV1alpha1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesClusterGeneratorV1alpha1Config) {
    super(scope, id, {
      manifest: { apiVersion: "generators.external-secrets.io/v1alpha1", kind: "ClusterGenerator", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesClusterGeneratorV1alpha1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
