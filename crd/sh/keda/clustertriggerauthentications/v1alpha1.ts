...
export class KubernetesClusterTriggerAuthenticationV1alpha1 extends Manifest {
  constructor(
    scope: Construct,
    id: string,
    { manifest, ...config }: KubernetesClusterTriggerAuthenticationV1alpha1Config,
  ) {
    super(scope, id, {
      manifest: { apiVersion: "keda.sh/v1alpha1", kind: "ClusterTriggerAuthentication", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesClusterTriggerAuthenticationV1alpha1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
