...
export class KubernetesGlobalRulesV1alpha1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesGlobalRulesV1alpha1Config) {
    super(scope, id, {
      manifest: { apiVersion: "monitoring.googleapis.com/v1alpha1", kind: "GlobalRules", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesGlobalRulesV1alpha1Config extends ManifestConfig {
  metadata: {
    annotations?: {
      [key: string]: string;
    };
