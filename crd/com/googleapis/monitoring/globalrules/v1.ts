...
export class KubernetesGlobalRulesV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesGlobalRulesV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "monitoring.googleapis.com/v1", kind: "GlobalRules", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesGlobalRulesV1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
