...
export class KubernetesClusterPodMonitoringV1alpha1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesClusterPodMonitoringV1alpha1Config) {
    super(scope, id, {
      manifest: { apiVersion: "monitoring.googleapis.com/v1alpha1", kind: "ClusterPodMonitoring", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesClusterPodMonitoringV1alpha1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
