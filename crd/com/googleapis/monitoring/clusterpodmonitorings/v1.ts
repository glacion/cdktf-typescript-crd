...
export class KubernetesClusterPodMonitoringV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesClusterPodMonitoringV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "monitoring.googleapis.com/v1", kind: "ClusterPodMonitoring", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesClusterPodMonitoringV1Config extends ManifestConfig {
  metadata: {
    annotations?: {
      [key: string]: string;
    };
