...
export class KubernetesPodMonitoringV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesPodMonitoringV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "monitoring.googleapis.com/v1", kind: "PodMonitoring", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesPodMonitoringV1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
