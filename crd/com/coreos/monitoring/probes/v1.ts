...
export class KubernetesProbeV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesProbeV1Config) {
    super(scope, id, { manifest: { apiVersion: "monitoring.coreos.com/v1", kind: "Probe", ...manifest }, ...config });
  }
}
export interface KubernetesProbeV1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
