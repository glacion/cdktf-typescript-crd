...
export class KubernetesPrometheusRuleV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesPrometheusRuleV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "monitoring.coreos.com/v1", kind: "PrometheusRule", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesPrometheusRuleV1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
