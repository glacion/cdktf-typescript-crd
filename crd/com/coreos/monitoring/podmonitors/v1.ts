...
export class KubernetesPodMonitorV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesPodMonitorV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "monitoring.coreos.com/v1", kind: "PodMonitor", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesPodMonitorV1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
