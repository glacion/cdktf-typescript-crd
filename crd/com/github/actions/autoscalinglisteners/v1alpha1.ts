...
export class KubernetesAutoscalingListenerV1alpha1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesAutoscalingListenerV1alpha1Config) {
    super(scope, id, {
      manifest: { apiVersion: "actions.github.com/v1alpha1", kind: "AutoscalingListener", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesAutoscalingListenerV1alpha1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
