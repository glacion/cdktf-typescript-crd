...
export class KubernetesAtlasDatabaseUserV1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesAtlasDatabaseUserV1Config) {
    super(scope, id, {
      manifest: { apiVersion: "atlas.mongodb.com/v1", kind: "AtlasDatabaseUser", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesAtlasDatabaseUserV1Config extends ManifestConfig {
  metadata: {
    annotations?: {
      [key: string]: string;
    };
