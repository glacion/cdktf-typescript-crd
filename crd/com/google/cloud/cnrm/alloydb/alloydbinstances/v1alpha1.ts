...
export class KubernetesAlloyDBInstanceV1alpha1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesAlloyDBInstanceV1alpha1Config) {
    super(scope, id, {
      manifest: { apiVersion: "alloydb.cnrm.cloud.google.com/v1alpha1", kind: "AlloyDBInstance", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesAlloyDBInstanceV1alpha1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
