...
export class KubernetesFirestoreDatabaseV1beta1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesFirestoreDatabaseV1beta1Config) {
    super(scope, id, {
      manifest: { apiVersion: "firestore.cnrm.cloud.google.com/v1beta1", kind: "FirestoreDatabase", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesFirestoreDatabaseV1beta1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
