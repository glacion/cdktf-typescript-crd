...
export class KubernetesPrivateCACAPoolV1beta1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesPrivateCACAPoolV1beta1Config) {
    super(scope, id, {
      manifest: { apiVersion: "privateca.cnrm.cloud.google.com/v1beta1", kind: "PrivateCACAPool", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesPrivateCACAPoolV1beta1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
