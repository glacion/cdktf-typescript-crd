...
export class KubernetesPubSubSubscriptionV1beta1 extends Manifest {
  constructor(scope: Construct, id: string, { manifest, ...config }: KubernetesPubSubSubscriptionV1beta1Config) {
    super(scope, id, {
      manifest: { apiVersion: "pubsub.cnrm.cloud.google.com/v1beta1", kind: "PubSubSubscription", ...manifest },
      ...config,
    });
  }
}
export interface KubernetesPubSubSubscriptionV1beta1Config extends ManifestConfig {
  manifest: {
    metadata: {
      annotations?: {
        [key: string]: string;
